...
public class Server
{
    ...
    public void chatApp(String username,int ack1)
    {
        ack = ack1;
        f.setLayout(null);
        ...
        s4.setBackground(new Color(8, 94, 81));
        ...
        f.setLocation(250, 50);        //Where the frame will appear, we can set that
    ...
    //To customise the message we are creating a method that takes a String 
    {
        ...
        panel.setLayout(new BoxLayout(panel,BoxLayout.Y_AXIS));               //create box layout object     that 
    ...
    public static void main(String[] args) {
        ...
        try
        {
            ServerSocket serv = new ServerSocket(6001);     //We have started a server on port 6001 using Class ServerSocket
            while(true)
            {
                Socket soc = serv.accept();   //We are accepting all the clients requesting to connect infinitely
                DataInputStream din = new DataInputStream(soc.getInputStream());    //Accepting any new messages
                dou = new DataOutputStream(soc.getOutputStream());    //For sending any new messages
                keyC = mg.keyExchange(din, dou);
                if(keyC > 126.0)
                {
                JOptionPane.showMessageDialog(f, "A new Client has connected..","New Connection"
        ...
        catch(Exception e)
        {
            ...
            f.repaint();
            e.printStackTrace();
            JOptionPane.showMessageDialog(f, "Client has gone offline..", "Connection Terminated",
                    JOptionPane.INFORMATION_MESSAGE);
        }
    }
}

class MessageCrypto extends Server
{
    ...
    public float keyExchange(DataInputStream din, DataOutputStream dio)
    {
        ...
        while (true) 
        {
            try 
            {
                ...
                sharedSecret = modPow(publicKey2, privateKey, n);
...
{
    int count;         //Check if any input is given or not
    ...
    public Authenticate()      //Method for automatic Login Page
    {
        ...
        //Now we will add the text field for username 
        ...
        //A textfield is created for taking inputs of the name.
    ...
    public void loginAction()
    {
        ...
        if(name.length() >= 1)
        {
            ...
            JOptionPane.showMessageDialog(frame, "Please enter again", "Invalid Input",JOptionPane.INFORMATION_MESSAGE);
        }
    }

    public void chatt(int count2) 
    {
